...
//			skill_unit_move(&sd->pd->bl,gettick(),1);
	...
	if (pc_checkskill(sd, SG_DEVIL) && pc_is_maxjoblv(sd))
		clif_status_load(&sd->bl, 504, 1);  //blindness [Komurka]
...
static inline uint8 clif_refineui_materials( struct item *item, struct item_data *id, struct refine_materials materials[REFINEUI_MAT_CNT] ){
	...
	if( item->refine >= 8 ){
		...
		if( item->refine >= 0 && item->refine <= 3 && clif_refineui_materials_sub( item, id, materials, count,  REFINE_COST_NORMAL ) ){
			count++;
		}
		...
		if( item->refine >= 4 && item->refine <= 6 && clif_refineui_materials_sub( item, id, materials, count, REFINE_COST_HD ) ){
			count++;
		}
	}

	// Enriched refine requirements
		if( item->refine >= 6 && item->refine <= 8 && clif_refineui_materials_sub( item, id, materials, count, REFINE_COST_ENRICHED ) ){
			count++;
	}
