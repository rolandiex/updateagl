			...
			default: {
				...
				switch(skill_id) {
					...
					case MG_FIREWALL:
						skillratio -= 50;
						break;
					case NSC_POISONSTORMGUST:
						skillratio += 40 * skill_lv;
						clif_specialeffect(&sd->bl, 237, AREA);
						break;
					case NSC_HOLYBOLT:
						...
						clif_specialeffect(&sd->bl, 928, AREA);
						break;
					case NSC_ELEMENTALBOLT:
						if (sc && sc->data[SC_SPELLFIST] && mflag&BF_SHORT) {
							skillratio += (sc->data[SC_SPELLFIST]->val4 * 100) + (sc->data[SC_SPELLFIST]->val1 * 50) - 100;// val4 = used bolt level, val2 = used spellfist level. [Rytech]
							ad.div_ = 1; // ad mods, to make it work similar to regular hits [Xazax]
							ad.flag = BF_WEAPON | BF_SHORT;
						...
						clif_specialeffect(&sd->bl, 182, AREA);
						break;
					case NSC_VENOMBOLT:
						if (sc && sc->data[SC_SPELLFIST] && mflag&BF_SHORT)  {
							skillratio += (sc->data[SC_SPELLFIST]->val4 * 100) + (sc->data[SC_SPELLFIST]->val1 * 50) - 100;// val4 = used bolt level, val2 = used spellfist level. [Rytech]
						...
						clif_specialeffect(&sd->bl, 192, AREA);
						break;
					case MG_FIREBOLT:
					case MG_COLDBOLT:
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "max_hp",                             &battle_config.max_hp,                        2500000,  100,    1000000000,     },
